...
class Data:
    def __init__(self,config):
        ...
        self.train_numbers,self.test_numbers = self.load_X_Y_numbers_liver(config['meta_path'])
        ...
        print "train_numbers:",len(self.train_numbers)
        print "test_numbers:",len(self.test_numbers)
    ...
    def load_X_Y_numbers_liver(self,meta_path):
        self.dicom_origin,self.liver_mask = organize_data.get_organized_data(meta_path,self.data_size[2]/2)
        numbers=[]
        train_numbers=[]
        test_numbers=[]
        print numbers
    ...
    def load_X_Y_train_batch_num(self):
        ...
        for number in self.train_numbers:
            for i in range(len(self.liver_mask[number])):
                total_num=total_num+1
                locs.append([number,i])
        return int(total_num/self.batch_size)-1,locs
    ...
    def load_X_Y_voxel_train_next_batch(self):
        ...
        for pair in temp_locs:
            ...
            Y_data_voxels.append(self.liver_mask[pair[0]][pair[1]])
        self.train_batch_index += 1
    ...
    def load_X_Y_voxel_test_next_batch(self,fix_sample=False):
        ...
        for i in idx:
            ...
            Y_test_voxels_batch.append(self.liver_mask[temp_pair[0]][temp_pair[1]])
        X_data = np.zeros([self.batch_size,self.data_size[0],self.data_size[1],self.data_size[2]],np.float32)
        Y_data = np.zeros([self.batch_size,self.data_size[0],self.data_size[1],self.data_size[2]],np.float32)
        '''
    ...
    def check_data(self):
        ...
        for pair in self.train_locs:
            ...
            shape2 = np.shape(self.liver_mask[pair[0]][pair[1]])
